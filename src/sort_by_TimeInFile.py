...
# Read the CSV file
input_file = "Files_above_length_8.csv"
...
df_sorted = df.sort_values(by=['FileID', 'TimeInFile'], ascending=[True, True])

# Reset index after sorting
df_sorted = df_sorted.reset_index(drop=True)

print("\nAfter sorting - Sample of first 10 rows:")
print(df_sorted[['FileID', 'TimeInFile', 'Filename']].head(10))

# Verify sorting worked correctly
print("\nVerification - checking a few FileIDs:")
for file_id in df_sorted['FileID'].unique()[:3]:
    file_data = df_sorted[df_sorted['FileID'] == file_id]
...
print(f"  Total FileIDs: {df_sorted['FileID'].nunique()}")
